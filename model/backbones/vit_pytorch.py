...
class TransReID(nn.Module):
    ...
    def forward(self, x, cam_label=None, view_label=None):
        x, x_patch, attn_weights = self.forward_features(x, cam_label, view_label)
        n, p, c = x_patch.shape
        x_patch = torch.reshape(x_patch, [n, int(x_patch.shape[1] / 8), 8, c])
        x_patch = x_patch.permute([0, 3, 1, 2])
        x_patch = x_patch.contiguous()
        return x, x_patch, attn_weights
