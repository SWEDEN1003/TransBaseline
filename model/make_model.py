...
class build_transformer(nn.Module):
    ...
    def get_cam(self, x, label=None, cam_label= None, view_label=None):
        ...
        attn_weights = torch.stack(attn_weights)        # 12 * B * H * N * N torch.Size([12, 64, 12, 129, 129])
...
class build_transformer_local(nn.Module):
    def __init__(self, num_classes, camera_num, view_num, cfg, factory, rearrange):
        ...
        self.b1norm = copy.deepcopy(layer_norm)
        self.b2 = copy.deepcopy(block)
        self.b2norm = copy.deepcopy(layer_norm)
    ...
    def forward(self, x, label=None, cam_label= None, view_label=None):  # label is unused if self.cos_layer == 'no'
        ...
        features, base, attn_weights  = self.base(x, cam_label=cam_label, view_label=view_label)

        # global branch
        b1_feat = self.b1norm(self.b1(features)[0]) # [64, 129, 768]
        ...
        b1_local_feat = self.b2norm(self.b2(torch.cat((token, b1_local_feat), dim=1))[0])
        local_feat_1 = b1_local_feat[:, 0]

        # lf_2
        b2_local_feat = x[:, patch_length:patch_length*2]
        b2_local_feat = self.b2norm(self.b2(torch.cat((token, b2_local_feat), dim=1))[0])
        local_feat_2 = b2_local_feat[:, 0]

        # lf_3
        b3_local_feat = x[:, patch_length*2:patch_length*3]
        b3_local_feat = self.b2norm(self.b2(torch.cat((token, b3_local_feat), dim=1))[0])
        local_feat_3 = b3_local_feat[:, 0]

        # lf_4
        b4_local_feat = x[:, patch_length*3:patch_length*4]
        b4_local_feat = self.b2norm(self.b2(torch.cat((token, b4_local_feat), dim=1))[0])
